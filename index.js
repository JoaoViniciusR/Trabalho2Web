...
document.querySelector('#myForm').addEventListener('submit', (event) => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const textFields = [
      ...
      document.querySelector('#textField2'),
      document.querySelector('#textField3')
    ];
  
    textFields.forEach(field => {
      const textField = mdc.textField.MDCTextField.attachTo(field);
    });
  
    const comboBox = mdc.select.MDCSelect.attachTo(document.querySelector('#comboBox1'));
  
    const checkbox = mdc.checkbox.MDCCheckbox.attachTo(document.querySelector('#check1'));
    ...
    buttons.forEach(button => {
      const buttonEl = mdc.button.MDCButton.attachTo(button);
    });
  });